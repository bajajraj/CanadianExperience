...
ViewEdit::ViewEdit(wxFrame* parent) :wxScrolledCanvas(parent, wxID_ANY)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnDialogMachine1, this, XRCID("Machine1"));
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnDialogMachine2, this, XRCID("Machine2"));
}
...
/**
 * Dialogue box for the machine 1
 * @param event the event attached
 */
void ViewEdit::OnDialogMachine1(wxCommandEvent& event)
{
    GetPicture()->Machine1DialogBox(this->GetParent());
    Refresh();
}

/**
 * Dialogue box for machine 2
 * @param event the event attached
 */
void ViewEdit::OnDialogMachine2(wxCommandEvent& event)
{
    GetPicture()->Machine2DialogBox(this->GetParent());
    Refresh();
}
