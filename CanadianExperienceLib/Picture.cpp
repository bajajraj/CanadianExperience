...
void Picture::Save(const wxString& filename)
{
    ...
    // Saving the machine XML files
    root->AddAttribute(L"machine1-num", wxString::Format(wxT("%i"), mMachine1->GetMachineNumber()));
    root->AddAttribute(L"machine2-num", wxString::Format(wxT("%i"), mMachine2->GetMachineNumber()));
    root->AddAttribute(L"machine1-start", wxString::Format(wxT("%i"), mMachine1->GetStart()));
    root->AddAttribute(L"machine2-start", wxString::Format(wxT("%i"), mMachine2->GetStart()));
...
void Picture::Load(const wxString& filename)
{
    ...
    // Loading the machines from XML
    mMachine1->SetMachineNumber(wxAtoi(root->GetAttribute(L"machine1-num", L"1")));
    mMachine2->SetMachineNumber(wxAtoi(root->GetAttribute(L"machine2-num", L"2")));
    mMachine1->SetStart(wxAtoi(root->GetAttribute(L"machine1-start", L"30")));
    mMachine2->SetStart(wxAtoi(root->GetAttribute(L"machine2-start", L"45")));
...
/**
 * Dialog box function for the machine 1
 * @param parent the parent window
 */
void Picture::Machine1DialogBox(wxWindow* parent)
{
    mMachine1->ShowDialogBox(parent);
}

/**
 * Dialog box function for the machine 2
 * @param parent the parent window
 */
void Picture::Machine2DialogBox(wxWindow* parent)
{
    mMachine2->ShowDialogBox(parent);
}
